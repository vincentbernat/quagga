...
zclient_read (struct thread *thread)
{
  ...
  switch (command)
	...
	(*zclient->local_vni_del) (command, zclient, length, vrf_id);
      break;
    case ZEBRA_MAC_ADD:
      if (zclient->local_mac_add)
        (*zclient->local_mac_add) (command, zclient, length, vrf_id);
      break;
    case ZEBRA_MAC_DEL:
      if (zclient->local_mac_del)
        (*zclient->local_mac_del) (command, zclient, length, vrf_id);
      break;
    default:
      break;
    }
